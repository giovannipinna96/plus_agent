...
# ============================================================================
# IN-MEMORY DATAFRAME STATE MANAGEMENT (TODO #2)
# ============================================================================

class DataFrameStateManager:
    """
    Manages in-memory DataFrame storage to eliminate redundant file I/O operations.

    This class provides a centralized state management system that:
    - Stores DataFrames in memory during an analysis session
    - Tracks the current active DataFrame
    - Provides load/save/clear operations
    - Eliminates the need for repeated pd.read_csv() calls

    Usage:
        # Load a dataset into memory
        df_manager.load_dataframe("data/titanic.csv")

        # Get the current DataFrame
        df = df_manager.get_current_dataframe()

        # Update the DataFrame after modifications
        df_manager.update_current_dataframe(modified_df)

        # Save results explicitly when needed
        df_manager.save_current_dataframe("output.csv")
    """
    ...
    def load_dataframe(self, file_path: str, force_reload: bool = False) -> pd.DataFrame:
        """
        Load a DataFrame from file into memory.
        ...
        if not force_reload and normalized_path in self._dataframes:
            self._current_key = normalized_path
            return self._dataframes[normalized_path]
        ...
        # Store in memory
        self._dataframes[normalized_path] = df
        self._current_key = normalized_path
    ...
    def get_current_dataframe(self) -> Optional[pd.DataFrame]:
        ...
        if self._current_key is None:
            return None
        return self._dataframes.get(self._current_key)

    def update_current_dataframe(self, df: pd.DataFrame):
        """Update the currently active DataFrame with a modified version."""
        if self._current_key is None:
            raise ValueError("No DataFrame currently loaded. Call load_dataframe() first.")

        self._dataframes[self._current_key] = df

        # Update metadata
        if self._current_key in self._metadata:
            self._metadata[self._current_key].update({
    ...
    def save_current_dataframe(self, output_path: str, index: bool = False) -> str:
        """
        ...
        if df is None:
            raise ValueError("No DataFrame currently loaded to save.")
    ...
    def clear_current(self):
        """Clear only the current DataFrame."""
        if self._current_key:
            self._dataframes.pop(self._current_key, None)
            self._metadata.pop(self._current_key, None)
            self._current_key = None

    def get_metadata(self, key: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Get metadata for a specific DataFrame or the current one."""
        target_key = key or self._current_key
        if target_key is None:
            return None
        return self._metadata.get(target_key)

    def has_dataframe(self, file_path: Optional[str] = None) -> bool:
        """Check if a DataFrame is loaded (either specific file or any DataFrame)."""
        if file_path:
            normalized_path = os.path.abspath(file_path)
            return normalized_path in self._dataframes
        return self._current_key is not None

# Global instance for use across all tools (public name for import *)
df_state_manager = DataFrameStateManager()
...
def read_csv_file(file_path: str) -> str:
    """
    Loads a CSV file into in-memory DataFrame and extracts essential dataset metadata.

    **IN-MEMORY OPTIMIZATION**: This tool loads the dataset into memory once, eliminating
    redundant file I/O for subsequent operations. All other tools will automatically use
    the in-memory DataFrame instead of re-reading from disk.
    ...
    """
    try:
        # Load DataFrame into memory using state manager (TODO #2)
        ...
        return f"CSV file loaded into memory successfully. Shape: {info['shape']}, Columns: {info['columns']}. Dataset is now cached for fast access."
...
def get_column_info(file_path: str, column_name: Optional[str] = None) -> str:
    ...
    **IN-MEMORY OPTIMIZATION**: Uses cached DataFrame if available, avoiding file I/O.
    ...
    """
    try:
        # Use in-memory DataFrame if available (TODO #2)
...
def handle_missing_values(file_path: str, column_name: str, method: str, fill_value: Optional[Union[str, float, int]] = None) -> str:
    ...
    **IN-MEMORY OPTIMIZATION**: Modifies the cached DataFrame in memory without disk I/O.
    ...
    Returns:
        Success message with count of imputed values and remaining missing values. Dataset is updated in memory automatically.
    """
    try:
        # Use in-memory DataFrame if available (TODO #2)
        df = df_state_manager.get_current_dataframe()
        ...
        if column_name not in df.columns:
            raise ColumnNotFoundError(
                problem=f"Column '{column_name}' does not exist in the DataFrame",
                context={"tool_name": "handle_missing_values", "parameters": {"column_name": column_name, "method": method}},
                cause=f"Available columns are: {list(df.columns)}",
                solution=f"Use one of the existing column names: {', '.join(df.columns[:5])}...",
                example=f"handle_missing_values(file_path, '{df.columns[0]}', 'mean')"
            )
        ...
        if method == "drop":
            ...
            if df[column_name].dtype in ['int64', 'float64']:
                ...
                raise DataTypeError(
                    problem=f"Cannot apply mean method to non-numeric column",
                    context={"tool_name": "handle_missing_values", "parameters": {"column_name": column_name, "method": "mean"}},
                    cause=f"Column '{column_name}' has data type '{df[column_name].dtype}' which is not numeric",
                    solution="Use 'mode' method for categorical columns or 'constant' with a specific value",
                    example=f"handle_missing_values(file_path, '{column_name}', 'mode')"
                )
        elif method == "median":
            if df[column_name].dtype in ['int64', 'float64']:
                ...
                raise DataTypeError(
                    problem=f"Cannot apply median method to non-numeric column",
                    context={"tool_name": "handle_missing_values", "parameters": {"column_name": column_name, "method": "median"}},
                    cause=f"Column '{column_name}' has data type '{df[column_name].dtype}' which is not numeric",
                    solution="Use 'mode' method for categorical columns or 'constant' with a specific value",
                    example=f"handle_missing_values(file_path, '{column_name}', 'mode')"
                )
        ...
        # Update the in-memory DataFrame (TODO #2)
        df_state_manager.update_current_dataframe(df)

        return f"✓ Handled {missing_count_before - missing_count_after} missing values in column '{column_name}' using method '{method}'. Remaining missing: {missing_count_after}. DataFrame updated in memory."
...
def create_dummy_variables(file_path: str, column_name: str, prefix: Optional[str] = None) -> str:
    ...
    **IN-MEMORY OPTIMIZATION**: Uses cached DataFrame if available and updates in memory without disk I/O.
    ...
    Returns:
        Success message with count and list of created dummy columns. DataFrame is updated in memory with new columns added.
    """
    try:
        # Use in-memory DataFrame if available (TODO #2)
        df = df_state_manager.get_current_dataframe()
        ...
        if column_name not in df.columns:
            raise ColumnNotFoundError(
                problem=f"Column '{column_name}' does not exist in the DataFrame",
                context={"tool_name": "create_dummy_variables", "parameters": {"column_name": column_name}},
                cause=f"Available columns are: {list(df.columns)}",
                solution=f"Use one of the existing column names: {', '.join(df.columns[:5])}...",
                example=f"create_dummy_variables(file_path, '{df.columns[0]}')"
            )
        ...
        return f"✓ Created {len(dummies.columns)} dummy variables for '{column_name}': {list(dummies.columns)[:5]}{'...' if len(dummies.columns) > 5 else ''}. DataFrame updated in memory with {len(df.columns)} total columns."
...
def modify_column_values(file_path: str, column_name: str, operation: str, value: Optional[Union[str, float, int]] = None) -> str:
    ...
    **IN-MEMORY OPTIMIZATION**: Uses cached DataFrame if available and updates in memory without disk I/O.
    ...
    Returns:
        Success message with count of modified values. DataFrame is updated in memory automatically.
    """
    try:
        # Use in-memory DataFrame if available (TODO #2)
        df = df_state_manager.get_current_dataframe()
        ...
        if column_name not in df.columns:
            raise ColumnNotFoundError(
                problem=f"Column '{column_name}' does not exist in the DataFrame",
                context={"tool_name": "modify_column_values", "parameters": {"column_name": column_name, "operation": operation}},
                cause=f"Available columns are: {list(df.columns)}",
                solution=f"Use one of the existing column names",
                example=f"modify_column_values(file_path, '{df.columns[0]}', 'multiply', 2)"
            )
        ...
        return f"✓ Modified {changed_count} values in column '{column_name}' using operation '{operation}'. DataFrame updated in memory."
...
def convert_data_types(file_path: str, column_name: str, target_type: str) -> str:
    ...
    **IN-MEMORY OPTIMIZATION**: Uses cached DataFrame if available and updates in memory without disk I/O.
    ...
    Returns:
        Success message showing original and new data types. DataFrame is updated in memory automatically.
    """
    try:
        # Use in-memory DataFrame if available (TODO #2)
        df = df_state_manager.get_current_dataframe()
        ...
        if column_name not in df.columns:
            raise ColumnNotFoundError(
                problem=f"Column '{column_name}' does not exist in the DataFrame",
                context={"tool_name": "convert_data_types", "parameters": {"column_name": column_name, "target_type": target_type}},
                cause=f"Available columns are: {list(df.columns)}",
                solution=f"Use one of the existing column names",
                example=f"convert_data_types(file_path, '{df.columns[0]}', 'float')"
            )
        ...
        return f"✓ Converted column '{column_name}' from {original_dtype} to {new_dtype}. DataFrame updated in memory."
...
def filter_data(file_path: str, column_name: str, condition: str, value: Union[str, float, int]) -> str:
    ...
    **IN-MEMORY OPTIMIZATION**: Uses cached DataFrame if available and updates in memory without disk I/O.
    The filtered DataFrame replaces the current DataFrame in memory.
    ...
    Returns:
        Success message with original and filtered row counts and applied condition description. DataFrame is updated in memory with filtered rows.
    """
    try:
        # Use in-memory DataFrame if available (TODO #2)
        df = df_state_manager.get_current_dataframe()
        ...
        if column_name not in df.columns:
            raise ColumnNotFoundError(
                problem=f"Column '{column_name}' does not exist in the DataFrame",
                context={"tool_name": "filter_data", "parameters": {"column_name": column_name, "condition": condition, "value": value}},
                cause=f"Available columns are: {list(df.columns)}",
                solution=f"Use one of the existing column names",
                example=f"filter_data(file_path, '{df.columns[0]}', 'greater_than', 10)"
            )
        ...
        if condition == "equals":
            ...
            raise InvalidOperatorError(
                problem=f"Invalid filter condition '{condition}'",
                context={"tool_name": "filter_data", "parameters": {"condition": condition}},
                cause=f"The condition '{condition}' is not recognized",
                solution="Use one of: 'equals', 'not_equals', 'greater_than', 'less_than', 'greater_equal', 'less_equal', 'contains'",
                example="filter_data(file_path, 'Age', 'greater_than', 18)"
            )

        filtered_rows = len(filtered_df)

        # Update the in-memory DataFrame with filtered data (TODO #2)
        df_state_manager.update_current_dataframe(filtered_df)

        return f"✓ Filtered data: {original_rows} → {filtered_rows} rows ({filtered_rows/original_rows*100:.1f}% retained) using condition '{column_name} {condition} {value}'. DataFrame updated in memory."
...
def perform_math_operations(file_path: str, operation: str, column1: str, column2: Optional[str] = None, value: Optional[float] = None) -> str:
    ...
    try:
        ...
        return f"Created new column '{new_col_name}' using operation '{operation}'. DataFrame updated in memory"
...
def string_operations(file_path: str, column_name: str, operation: str, parameter: Optional[str] = None) -> str:
    ...
    try:
        ...
        # Save the modified dataframe
        return f"Applied string operation '{operation}' to column '{column_name}'. Result: {new_col_name}. DataFrame updated in memory"
...
def drop_column(file_path: str, column_name: str) -> str:
    ...
    try:
        ...
        return f"Dropped column '{column_name}'. New shape: {df.shape}. DataFrame updated in memory"
    except Exception as e:
...
def drop_null_rows(file_path: str, column_name: Optional[str] = None) -> str:
    ...
    try:
        ...
        return f"Dropped {dropped} rows with nulls. New shape: {df.shape}. DataFrame updated in memory"
    except Exception as e:
...
def fill_numeric_nulls(file_path: str, column_name: str, method: str = "mean") -> str:
    ...
    try:
        ...
        df[column_name] = df[column_name].fillna(fill_value)        
        return f"Filled {null_count} nulls in '{column_name}' with {method} ({fill_value:.2f}). DataFrame updated in memory"
    except Exception as e:
...
def fill_categorical_nulls(file_path: str, column_name: str, method: str = "mode") -> str:
    ...
    try:
        ...
        df[column_name] = df[column_name].fillna(fill_value)        
        return f"Filled {null_count} nulls in '{column_name}' with '{fill_value}'. DataFrame updated in memory"
    except Exception as e:
...
def encode_categorical(file_path: str, column_name: str, encoding_type: str = "onehot") -> str:
    ...
    try:
        ...
        return f"{msg}. DataFrame updated in memory"
    except Exception as e:
...
def create_new_feature(file_path: str, new_column: str, column1: str, operation: str, column2_or_value: Union[str, float]) -> str:
    ...
    try:
        ...
        if operation == '+':
            ...
            return f"Unknown operation: {operation}"        
        return f"Created new feature '{new_column}' using {column1} {operation} {column2_or_value}. DataFrame updated in memory"
    except Exception as e:
...
def filter_rows_numeric(file_path: str, column_name: str, operator: str, value: float) -> str:
    ...
    try:
        ...
        return f"Filtered {original_len} -> {len(df_filtered)} rows where {column_name} {operator} {value}. DataFrame updated in memory"
    except Exception as e:
...
def filter_rows_categorical(file_path: str, column_name: str, values: str, include: bool = True) -> str:
    ...
    try:
        ...
        return f"Filtered {original_len} -> {len(df_filtered)} rows. {action}: {value_list}. DataFrame updated in memory"
    except Exception as e:
        return f"Error filtering: {str(e)}"
...
def select_columns(file_path: str, columns: str) -> str:
    ...
    try:
        ...
        return f"Selected {len(col_list)} columns. New shape: {df_selected.shape}. DataFrame updated in memory"
    except Exception as e:
...
def create_histogram(file_path: str, column_name: str, bins: int = 20) -> str:
    ...
    try:
        ...
        return f"Histogram created for '{column_name}'. {stats_info}. DataFrame updated in memory"
    except Exception as e:
...
def create_scatter_plot(file_path: str, x_column: str, y_column: str, color_column: Optional[str] = None) -> str:
    ...
    try:
        ...
        return f"Scatter plot created. Correlation: {corr:.3f}. DataFrame updated in memory"
    except Exception as e:
...
def create_bar_chart(file_path: str, column_name: str, top_n: int = 10) -> str:
    ...
    try:
        ...
        return f"Bar chart created for '{column_name}'. Top value: {value_counts.index[0]} ({value_counts.iloc[0]} occurrences). DataFrame updated in memory"
    except Exception as e:
...
def create_correlation_heatmap(file_path: str, columns: Optional[str] = None) -> str:
    ...
    try:
        ...
        return f"Correlation heatmap created for {corr_matrix.shape[0]} variables. DataFrame updated in memory"
    except Exception as e:
